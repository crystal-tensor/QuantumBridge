...
"""Run the Stage 9H bidirectional bridge equivalence example."""
...
from quantumbridge.compat.pennylane_qiskit.examples import run_bridge_equivalence_example


def main() -> None:
    results = run_bridge_equivalence_example()
    native = results["native"]
    upstream = results["upstream"]
    print("QuantumBridge native PennyLane-Qiskit bridge equivalence")
    print(f"equivalence_status: {native.equivalence_status}")
    print(f"probabilities: {native.statevector_probabilities}")
    print(f"counts: {native.counts}")
    print(f"max_probability_delta: {native.circuit_summary['max_probability_delta']}")
