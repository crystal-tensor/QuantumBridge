...
"""Optional dependency helpers for qiskit-dynamics."""
...
def dependency_available() -> bool:
    return importlib.util.find_spec("qiskit_dynamics") is not None
...
def get_upstream_version() -> str | None:
    if not dependency_available():
        return None
    try:
        return metadata.version("qiskit-dynamics")
...
def validate_dynamics_dependencies() -> dict[str, object]:
    available = dependency_available()
    return {
        "dependency": "qiskit-dynamics",
        "available": available,
